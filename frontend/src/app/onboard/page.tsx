...
export default function OnboardPage() {
  ...
  // Sandbox (enabled by default for full functionality; user can disable if needed)
  const [sandboxEnabled, setSandboxEnabled] = useState(true);
  const [sandboxBackend, setSandboxBackend] = useState("docker");
  const [sandboxDockerImage, setSandboxDockerImage] = useState("openagent-sandbox-office:2026.07");
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Recommended — enables code execution in a secure Docker container.
            </p>
          </div>
          <Switch checked={props.enabled} onCheckedChange={props.setEnabled} />
        </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => props.setDockerImage(e.target.value)}
                      placeholder="openagent-sandbox-office:2026.07"
                    className="font-mono text-sm"
                  />
                </div>
              )}
